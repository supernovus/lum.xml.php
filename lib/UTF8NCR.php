...
class UTF8NCR
{
   ...
   */
  public static function encode ($content, $quotes=false) 
  {
    $content = self::encode_xml_chars($content, $quotes);
    $contents = self::to_array($content);
    $swap = "";
    $iCount = count($contents);
    for ($o=0;$o<$iCount;$o++) 
    {
      $contents[$o] = self::encode_utf8_char($contents[$o]);
      $swap .= $contents[$o];
    }
   ...
   * @return string             The encoded string.
   */
  public static function encode_xml_chars ($string, $quotes=false)
  { 
    $string = str_replace('<', '&#x3C;', $string);
    $string = str_replace('>', '&#x3E;', $string);
    $string = self::fix_ampersand($string);
    if ($quotes)
    { 
      $string = str_replace("'", '&#x27;', $string);
      $string = str_replace('"', '&#x22;', $string);
   ...
   * @return string             The encoded string.
   */
  public static function fix_ampersand ($string)
  { 
